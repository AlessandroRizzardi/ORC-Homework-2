...
if __name__=='__main__':
    ...
    if conf.SELECTION_MATRIX==1:
        underact = 0                
        img_suffix = img_suffix + 'selmat_'                                # underactuation penalty weight
    elif conf.ACTUATION_PENALTY==1:
        underact = 1e6               
        img_suffix = img_suffix + 'undact_'                                     
    ...
    if conf.PLOT_TORQUES: 
        ...
        plt.savefig(img_suffix + 'torque.eps')
    ...
    if conf.PLOT_JOINT_POS:         
        ...
        plt.savefig(img_suffix + 'position.eps')
    ...
    if conf.PLOT_JOINT_VEL:         
        ...
        plt.savefig(img_suffix + 'velocity.eps')
    
    print('Run > solver.start_simu(x0, X, U, KK, conf.dt_sim, conf.PUSH, conf.TORQUE_LIMITS) to restart simulation')
    plt.show()
